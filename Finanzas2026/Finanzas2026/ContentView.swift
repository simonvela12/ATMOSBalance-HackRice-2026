...
struct ContentView: View {
    ...
    @State private var profile = CleanData.profile(currentCash: nil, transactions: [], preserving: nil)
...
private enum CleanTab: Hashable { case home, calendar, plan, whatIf, context }
...
private struct CleanHomeView: View {
    ...
    var body: some View {
        CleanScreen(title: "Your money, with context", subtitle: linked ? "Bank activity is factual. Everything in your future plan is yours to add or edit." : "Connect a bank, then tell the app what the bank cannot know about the future.") {
            bankStatusCard
            ...
            if let dashboard {
                ...
                HStack(spacing: 12) {
                    ...
                    CleanMetric(title: "Protected", value: max(0, profile.currentCash - dashboard.safeToSpendNow))
                }
...
private struct CleanCalendarView: View {
    ...
    var body: some View {
        CleanScreen(title: "Future cash path", subtitle: "See when your plan gets comfortable, tight, or unsafe.") {
            if !linked {
                CleanEmptyCard(icon: "calendar", title: "Connect a bank to build the path", text: "The calendar uses your real cash balance plus the future items you add.")
            } else if CleanData.upcoming(profile: profile).isEmpty {
                CleanEmptyCard(icon: "calendar.badge.plus", title: "No future items yet", text: "Add expected money, bills, reimbursements, or goals from Context.")
            } else {
                ForEach(CleanData.upcoming(profile: profile)) { item in
                    HStack(spacing: 12) {
                        VStack(spacing: 2) {
                            ...
                            Text(item.date, format: .dateTime.day()).font(.headline)
                        }
                        .frame(width: 42)
                        VStack(alignment: .leading, spacing: 3) {
                            Text(item.title).font(.headline)
                            Text(item.detail).font(.caption).foregroundStyle(.white.opacity(0.5))
                        }
                        Spacer()
                ...
                if let tightest = points.min(by: { $0.projectedCash < $1.projectedCash }) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Text(tightest.date, format: .dateTime.month(.wide).day())
...
private struct CleanPlanView: View {
    ...
    private var futureIncome: [IncomeEvent] {
        profile.incomeEvents.filter { $0.date > profile.asOfDate }.sorted { $0.date < $1.date }
    }
    private var futureExpenses: [ExpenseEvent] {
        profile.expenseEvents.filter { $0.date > profile.asOfDate }.sorted { $0.date < $1.date }
    }

    var body: some View {
        CleanScreen(title: "Your editable plan", subtitle: "Bank transactions stay factual. Future assumptions can be changed or removed at any time.") {
            reserveCard

            if futureIncome.isEmpty && futureExpenses.isEmpty && profile.goals.isEmpty {
                CleanEmptyCard(icon: "list.bullet.rectangle", title: "Your plan is empty", text: "Add what you expect to happen with your money in plain language.")
            }

            if !futureIncome.isEmpty {
                planSection(title: "EXPECTED MONEY") {
                    ForEach(futureIncome) { event in
                        planRow(title: event.source, amount: event.amount, date: event.date, positive: true, detail: CleanData.incomeDetail(event)) {
            ...
            if !futureExpenses.isEmpty {
                planSection(title: "UPCOMING EXPENSES") {
                    ForEach(futureExpenses) { event in
                        planRow(title: event.category, amount: event.amount, date: event.date, positive: false, detail: CleanData.expenseDetail(event)) {
            ...
            if !profile.goals.isEmpty {
                planSection(title: "GOALS") {
                    ForEach(profile.goals) { goal in
                        planRow(title: goal.name, amount: goal.remainingAmount, date: goal.deadline, positive: false, detail: goal.priority == .mandatory ? "Must happen" : "Flexible") {
            ...
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(amount, format: .currency(code: "USD").precision(.fractionLength(0)))
                        .font(.title2.weight(.bold))
                    Text(amount > 0 ? "Tap to edit the cash you do not want to touch." : "Tap to set money you want to keep untouched.")
                        .font(.caption).foregroundStyle(.white.opacity(0.52))
                }
                Spacer()
                Image(systemName: "pencil.circle.fill").font(.title2).foregroundStyle(.white.opacity(0.7))
            }
            .padding(18)
            .cleanCard()
        }
    ...
    private func planRow(title: String, amount: Double, date: Date, positive: Bool, detail: String, edit: @escaping () -> Void) -> some View {
        Button(action: edit) {
            HStack(spacing: 12) {
                VStack(alignment: .leading, spacing: 3) {
                ...
                VStack(alignment: .trailing, spacing: 5) {
                    Text(amount, format: .currency(code: "USD").precision(.fractionLength(0)))
                        .font(.headline).foregroundStyle(positive ? .green : .orange)
                    Text("Edit").font(.caption.weight(.semibold)).foregroundStyle(.white.opacity(0.55))
                }
            }
            .padding(16)
            .cleanCard()
...
private struct CleanWhatIfView: View {
    ...
    var body: some View {
        CleanScreen(title: "What if I spend this?", subtitle: "Try a purchase without changing your plan.") {
            if !linked {
                CleanEmptyCard(icon: "cart", title: "Connect a bank first", text: "What If needs a real current cash balance.")
            } else {
                VStack(alignment: .leading, spacing: 10) {
                    Text("PURCHASE AMOUNT").cleanEyebrow()
                    HStack {
                        Text("$").font(.title2).foregroundStyle(.white.opacity(0.55))
                ...
                if let analysis {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        HStack {
                            Text("Tightest date").foregroundStyle(.white.opacity(0.55))
                            Spacer()
...
private struct CleanContextView: View {
    ...
    var body: some View {
        CleanScreen(title: "Tell us what the bank can't know", subtitle: "Write what you expect in normal English or Spanish. You review every detail before it changes the plan.") {
            VStack(alignment: .leading, spacing: 12) {
            ...
            CleanEmptyCard(
                ...
                title: "Nothing is guessed silently",
                text: "If amount, date, or type is unclear, you fill it in before saving. You can edit or delete it later from Plan."
            )
        }
    }

    private func analyze() {
        let draft = NaturalLanguagePlanningInterpreter.parse(text, asOfDate: profile.asOfDate, calendar: CleanData.calendar)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if form.kind != .reserve {
                if form.hasDate {
                    DatePicker("When", selection: $form.date, in: asOfDate..., displayedComponents: .date)
                    Button("Remove date") { form.hasDate = false }
                        .font(.caption)
                } else {
                    HStack {
                        Label("A date is still needed", systemImage: "calendar.badge.exclamationmark")
                            .font(.subheadline).foregroundStyle(.yellow)
                        Spacer()
                        Button("Add date") {
                            form.date = CleanData.calendar.date(byAdding: .day, value: 1, to: asOfDate) ?? asOfDate
                            form.hasDate = true
                        }
                    }
                }
            ...
            if form.kind == .expense {
                Toggle("I have to pay this", isOn: $form.committed)
...
private struct CleanPlanEditor: View {
    ...
    init(profile: Binding<FinancialProfile>, target: PlanEditorTarget) {
        ...
        case .goal(let id):
            ...
            _date = State(initialValue: goal.deadline)
        ...
        case .reserve:
            let reserve = FinancialEngine.personalReserve(profile: p, on: p.asOfDate)
            _title = State(initialValue: "Cash reserve")
            _amountText = State(initialValue: cleanNumber(reserve))
            _date = State(initialValue: p.asOfDate)
            _cadence = State(initialValue: nil)
            _confidence = State(initialValue: 1)
            _committed = State(initialValue: true)
            _essential = State(initialValue: true)
            ...
            Form {
                if case .reserve = target {
                    ...
                    Section("Details") {
                        ...
                        TextField("Amount", text: $amountText).keyboardType(.decimalPad)
                        DatePicker("Date", selection: $date, in: profile.asOfDate..., displayedComponents: .date)
                    }
                }
                ...
                case .expense:
                    Section("Expense") {
                        ...
                        Toggle("I have to pay this", isOn: $committed)
                ...
                Section {
                    Button("Save changes") { saveChanges() }
                        .disabled((amount ?? -1) < 0 || (targetNeedsTitle && title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty))
                    if case .reserve = target {
                        ...
                        Button("Delete from plan", role: .destructive) { deleteTarget(); dismiss() }
                    }
                }
            }
    ...
    private func saveChanges() {
        ...
        case .income(let id):
            ...
            CleanData.addIncome(title: trimmed, amount: amount, date: date, cadence: cadence, confidence: confidence, to: &profile)
        case .expense(let id):
            guard let old = profile.expenseEvents.first(where: { $0.id == id }) else { return }
            CleanData.removeExpenseSeries(old, from: &profile)
            CleanData.addExpense(title: trimmed, amount: amount, date: date, cadence: cadence, committed: committed, essential: essential, to: &profile)
        case .goal(let id):
            guard let index = profile.goals.firstIndex(where: { $0.id == id }) else { return }
            profile.goals[index] = Goal(id: id, name: trimmed, targetAmount: amount, amountAlreadyPaid: profile.goals[index].amountAlreadyPaid, deadline: date, priority: goalPriority)
        case .reserve:
            profile.personalReserveSteps = [PersonalReserveStep(effectiveDate: date, minimumCash: amount, note: "Personal reserve")]
        }
        dismiss()
    }
    ...
    private func deleteTarget() {
        switch target {
        case .income(let id):
            if let event = profile.incomeEvents.first(where: { $0.id == id }) { CleanData.removeIncomeSeries(event, from: &profile) }
        case .expense(let id):
            if let event = profile.expenseEvents.first(where: { $0.id == id }) { CleanData.removeExpenseSeries(event, from: &profile) }
        case .goal(let id): profile.goals.removeAll { $0.id == id }
        case .reserve: profile.personalReserveSteps.removeAll()
        }
    }
}
...
private struct ContextForm {
    ...
    init(draft: NaturalPlanDraft, asOfDate: Date) {
        ...
        date = draft.date ?? (CleanData.calendar.date(byAdding: .day, value: 1, to: asOfDate) ?? asOfDate)
        hasDate = draft.kind == .reserve || draft.date != nil
    ...
    var canSave: Bool {
        ...
        if kind != .reserve && !hasDate { return false }
        return true
    }
}
...
private enum CleanData {
    ...
    static func horizon(from date: Date) -> Date { calendar.date(byAdding: .day, value: 90, to: date) ?? date }
    ...
    static func profile(currentCash: Double?, transactions: [FinanceCore.FinancialTransaction], preserving old: FinancialProfile?) -> FinancialProfile {
        let asOf = day(Date())
        let usable = transactions.filter { !$0.isPending && !$0.isTransfer && $0.transactionDate <= asOf }
        let historicalIncome = usable.filter { $0.direction == .inflow }.map { tx in
            IncomeEvent(
        ...
        let historicalExpenses = usable.filter { $0.direction == .outflow }.map { tx in
            ExpenseEvent(
                amount: abs(Double(tx.amountMinorUnits)) / 100,
                date: day(tx.transactionDate),
                category: label(tx),
        ...
        let manualIncome = old?.incomeEvents.filter { $0.date > asOf && $0.planningSource == .manual } ?? []
        let manualExpenses = old?.expenseEvents.filter { $0.date > asOf && $0.planningSource == .manual } ?? []
        let reserves = old?.personalReserveSteps ?? []
        let goals = old?.goals ?? []

        return FinancialProfile(
            currentCash: currentCash ?? 0,
            asOfDate: asOf,
            personalReserveSteps: reserves,
            ...
            spendingPolicy: old?.spendingPolicy ?? SpendingPolicy(lookbackWeeks: 6, bufferWeeks: 2, manualMinimumBuffer: 0)
        )
    }
    ...
    static func apply(form: ContextForm, to profile: inout FinancialProfile) -> Bool {
        ...
        switch kind {
        case .income:
            guard form.hasDate else { return false }
            addIncome(title: title, amount: amount, date: form.date, cadence: form.cadence, confidence: form.confidence, to: &profile)
        case .expense:
            guard form.hasDate else { return false }
            addExpense(title: title, amount: amount, date: form.date, cadence: form.cadence, committed: form.committed, essential: form.essential, to: &profile)
        case .goal:
            guard form.hasDate else { return false }
            profile.goals.append(Goal(name: title, targetAmount: amount, deadline: form.date, priority: form.goalPriority))
        case .reserve:
            profile.personalReserveSteps = [PersonalReserveStep(effectiveDate: profile.asOfDate, minimumCash: amount, note: "Personal reserve")]
        }
        return true
    }

    static func addIncome(title: String, amount: Double, date: Date, cadence: RecurrenceCadence?, confidence: Double, to profile: inout FinancialProfile) {
        let dates = occurrenceDates(first: date, cadence: cadence, through: horizon(from: profile.asOfDate))
        let rule = cadence.map { RecurrenceRule(cadence: $0, firstOccurrence: date) }
        for occurrence in dates where occurrence > profile.asOfDate {
            profile.incomeEvents.append(IncomeEvent(
                amount: amount,
                date: occurrence,
                source: title,
    ...
    static func addExpense(title: String, amount: Double, date: Date, cadence: RecurrenceCadence?, committed: Bool, essential: Bool, to profile: inout FinancialProfile) {
        let dates = occurrenceDates(first: date, cadence: cadence, through: horizon(from: profile.asOfDate))
        let rule = cadence.map { RecurrenceRule(cadence: $0, firstOccurrence: date) }
        for occurrence in dates where occurrence > profile.asOfDate {
            profile.expenseEvents.append(ExpenseEvent(
                amount: amount,
                date: occurrence,
                category: title,
                essential: essential,
    ...
    static func removeIncomeSeries(_ event: IncomeEvent, from profile: inout FinancialProfile) {
        if let first = event.recurrenceRule?.firstOccurrence {
            profile.incomeEvents.removeAll {
                $0.planningSource == .manual && $0.source == event.source && $0.recurrenceRule?.firstOccurrence == first
            }
        } else {
    ...
    static func removeExpenseSeries(_ event: ExpenseEvent, from profile: inout FinancialProfile) {
        if let first = event.recurrenceRule?.firstOccurrence {
            profile.expenseEvents.removeAll {
                $0.planningSource == .manual && $0.category == event.category && $0.recurrenceRule?.firstOccurrence == first
            }
        } else {
    ...
    static func occurrenceDates(first: Date, cadence: RecurrenceCadence?, through end: Date) -> [Date] {
        guard let cadence else { return [day(first)] }
        return (try? FinancialScheduleBuilder.dates(startingOn: day(first), through: end, cadence: cadence, calendar: calendar)) ?? [day(first)]
    }

    static func upcoming(profile: FinancialProfile) -> [UpcomingItem] {
        let incomes = profile.incomeEvents.filter { $0.date > profile.asOfDate }.map {
            UpcomingItem(id: "i-\($0.id)", date: $0.date, title: $0.source, amount: $0.adjustedAmount, detail: incomeDetail($0))
        }
        let expenses = profile.expenseEvents.filter { $0.date > profile.asOfDate && $0.committed }.map {
            UpcomingItem(id: "e-\($0.id)", date: $0.date, title: $0.category, amount: -$0.amount, detail: expenseDetail($0))
        }
        let goals = profile.goals.filter { $0.remainingAmount > 0 }.map {
            UpcomingItem(id: "g-\($0.id)", date: $0.deadline, title: $0.name, amount: -$0.remainingAmount, detail: $0.priority == .mandatory ? "Must happen" : "Flexible goal")
        }
    ...
    static func expenseDetail(_ event: ExpenseEvent) -> String {
        let payment = event.committed ? "Must pay" : "Optional"
        if event.recurrenceRule != nil { return "\(payment) · \(recurrenceText(event.recurrenceRule))" }
        return payment
    }
        ...
        return grouped
            ...
            .map { key, values in
                WeeklySpendingSample(
                    weekStart: key,
                    totalVariableSpending: values.reduce(0) { $0 + abs(Double($1.amountMinorUnits)) / 100 }
                )
            }
            .sorted { $0.weekStart < $1.weekStart }
    }

    private static func label(_ tx: FinanceCore.FinancialTransaction) -> String {
        let value = (tx.merchantName ?? tx.transactionDescription).trimmingCharacters(in: .whitespacesAndNewlines)
...
private struct CleanScreen<Content: View>: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                LinearGradient(
                    colors: [Color(red: 0.03, green: 0.065, blue: 0.12), Color(red: 0.045, green: 0.11, blue: 0.18), Color(red: 0.02, green: 0.045, blue: 0.09)],
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                Text(title).font(.headline)
                Text(text).font(.caption).foregroundStyle(.white.opacity(0.52))
            }
        }
        .padding(16)
        .cleanCard()
...
private struct CleanPrimaryButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(.white.opacity(configuration.isPressed ? 0.78 : 1), in: RoundedRectangle(cornerRadius: 13))
    }
}
...
private extension View {
    ...
    func cleanEyebrow() -> some View {
        font(.caption.weight(.bold)).tracking(1.1).foregroundStyle(.white.opacity(0.55))
    }
}
...
private func cleanHealthExplanation(_ status: FinancialHealthStatus) -> String {
    switch status {
    case .safe: return "Your future cash path stays above the protection you set."
    case .tight: return "Your plan stays possible, but the cushion gets thin at least once."
    case .notSafe: return "At least one future point drops below a protected minimum."
    }
}

private func cleanPurchaseTitle(_ status: PurchaseStatus) -> String {
    switch status {
    case .safe: return "This purchase fits your plan"
    case .tight: return "Possible, but it makes things tight"
...
private func cleanPurchaseReason(_ reason: PurchaseDecisionReason) -> String {
    switch reason {
    case .preservesRecommendedBuffer: return "You stay above the recommended cushion across the full future path."
    case .usesSafetyBuffer: return "You use part of the cushion, but stay above your hard minimum."
    case .violatesPersonalReserve: return "You would dip below the cash reserve you chose."
    case .violatesInstitutionalMinimum: return "You would dip below an account minimum."
    case .violatesMultipleHardConstraints: return "You would cross more than one protected minimum."
    }
}
