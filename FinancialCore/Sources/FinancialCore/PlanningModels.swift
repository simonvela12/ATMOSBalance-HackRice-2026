...
public enum CashFlowDirection: String, Codable, Sendable { case income, expense }
...
public struct AmountRange: Codable, Equatable, Sendable {
    ...
    public func amount(for scenario: FinancialScenario, direction: CashFlowDirection) -> Double {
        ...
        case (.income, .conservative): minimum
        case (.income, .expected): expected
        case (.income, .optimistic): maximum
        case (.expense, .conservative): maximum
        case (.expense, .expected): expected
        case (.expense, .optimistic): minimum
        }
    }
}
...
public struct DateWindow: Codable, Equatable, Sendable {
    ...
    public func date(for scenario: FinancialScenario, direction: CashFlowDirection) -> Date {
        switch (direction, scenario) {
        case (.income, .conservative): latest
        case (.income, .expected): expected
        case (.income, .optimistic): earliest
        case (.expense, .conservative): earliest
        case (.expense, .expected): expected
        case (.expense, .optimistic): latest
        }
    }

    public func contains(_ date: Date) -> Bool { date >= earliest && date <= latest }
}
...
public enum PlanningRecurrenceCadence: Codable, Equatable, Sendable {
    ...
    public init(_ cadence: RecurrenceCadence) {
        switch cadence { case .weekly: self = .weekly; case .biweekly: self = .biweekly; case .monthly: self = .monthly }
    }
    ...
    fileprivate func nextDate(after date: Date, calendar: Calendar) -> Date? {
        ...
        case .custom(let interval):
            ...
            case .day: calendar.date(byAdding: .day, value: interval.value, to: date)
            case .week: calendar.date(byAdding: .day, value: interval.value * 7, to: date)
            case .month: calendar.date(byAdding: .month, value: interval.value, to: date)
            }
        }
    }
}
...
public struct RecurrenceRule: Codable, Equatable, Sendable {
    ...
    public init(cadence: PlanningRecurrenceCadence, firstOccurrence: Date, endDate: Date? = nil, isPaused: Bool = false) {
        self.cadence = cadence; self.firstOccurrence = firstOccurrence; self.endDate = endDate; self.isPaused = isPaused
    }
    ...
    public func occurrenceDates(through horizon: Date, calendar: Calendar = .current) -> [Date] {
        ...
        var result: [Date] = [], current = firstOccurrence, occurrenceIndex = 0
        while current <= horizon {
            ...
            let next: Date?
            switch cadence {
            case .weekly: next = calendar.date(byAdding: .day, value: occurrenceIndex * 7, to: firstOccurrence)
            case .biweekly: next = calendar.date(byAdding: .day, value: occurrenceIndex * 14, to: firstOccurrence)
            case .monthly: next = calendar.date(byAdding: .month, value: occurrenceIndex, to: firstOccurrence)
            case .custom(let interval):
                switch interval.unit {
                case .day: next = calendar.date(byAdding: .day, value: occurrenceIndex * interval.value, to: firstOccurrence)
                case .week: next = calendar.date(byAdding: .day, value: occurrenceIndex * interval.value * 7, to: firstOccurrence)
                case .month: next = calendar.date(byAdding: .month, value: occurrenceIndex * interval.value, to: firstOccurrence)
                }
            }
...
public struct IncomeAllocation: Codable, Equatable, Identifiable, Sendable {
    ...
    public init(id: UUID = UUID(), amount: Double, purpose: String, linkedGoalID: UUID? = nil) {
        self.id = id; self.amount = max(0, amount); self.purpose = purpose; self.linkedGoalID = linkedGoalID
    }
}
...
public extension IncomeEvent {
    func scenarioNominalAmount(for scenario: FinancialScenario) -> Double { amountRange?.amount(for: scenario, direction: .income) ?? max(0, amount) }
    func scenarioDate(for scenario: FinancialScenario) -> Date { dateWindow?.date(for: scenario, direction: .income) ?? date }
}

public extension ExpenseEvent {
    func scenarioAmount(for scenario: FinancialScenario) -> Double { amountRange?.amount(for: scenario, direction: .expense) ?? max(0, amount) }
    func scenarioDate(for scenario: FinancialScenario) -> Date { dateWindow?.date(for: scenario, direction: .expense) ?? date }
}
